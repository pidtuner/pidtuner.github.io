...
const PidWorker = {
    ...
    makeSimulation(data) {
		...
		arma_gains.set_at(5, 0, new Arma.cx_double(data.du_lim, 0.0));
		// get ts
		var ts_real = data.uniform_time[1] - data.uniform_time[0];
		var ts      = Arma.CxMat.zeros(1, 1);
		ts.set_at(0, 0, new Arma.cx_double(ts_real, 0.0));
		...
		limits.set_at(2, 0, new Arma.cx_double(-data.du_lim  , 0.0));
		limits.set_at(3, 0, new Arma.cx_double(+data.du_lim  , 0.0));
		// get simulation time
